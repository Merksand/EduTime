...
function ejecutarCarrera() {
    ...
    let seleccion = document.querySelector("#filtrar-carrera");
    // console.log('3');
    if (seleccion) { 
      seleccion.addEventListener("click", () => {
        let seleccionValor = document.querySelector("#filtrar-carrera").value;
        console.log(seleccionValor); 
        const carreraFiltro = filtroPrincipal.filter((carrera) => {
            if (seleccionValor === carrera.materia) {
                return carrera.materia === seleccionValor
                
          } else if(seleccionValor === 'Todas') {
              return true; 
            }
          });
          console.log(carreraFiltro);
          mostrarProfesores(carreraFiltro);
      });
