...
function ejecutarProfesores() {
    const filtroPrincipal = [
        ...
        { nombre: 'María', apellido: 'González', materia: 'Electricidad Industrial', aula: 'Aula 146', horario: ['Lunes: 16:00 - 18:00', 'Miércoles: 18:00 - 20:00'] },{ nombre: 'Marta', apellido: 'Gómez', materia: 'Química', aula: 'Aula 201', horario: ['Lunes: 8:00 - 10:00', 'Miércoles: 10:00 - 12:00'] },
    ...
    let btn = document.querySelector("#btn-filtrar-profesores");
    if (btn) { 
      btn.addEventListener("click", () => {
        let nombreDocente = document.querySelector("#nombre").value;
        let apellidoDocente = document.querySelector("#apellido").value;
    
        // console.log(filtroPrincipal); 
        const docente = filtroPrincipal.filter((lista) => {
          if (nombreDocente === lista.nombre || apellidoDocente === lista.apellido) {
              return lista.nombre === nombreDocente ||  lista.apellido === apellidoDocente;
              
        } else {
            return false; 
          }
        });
        const profesoresFiltrados = filtroPrincipal.filter(profesor => {
            return profesor.nombre.toLowerCase().includes(nombreDocente); // Filtrar por el nombre del profesor
          });
        console.log(docente);
        // console.log(profesoresFiltrados);
        mostrarProfesores(docente);
      });
